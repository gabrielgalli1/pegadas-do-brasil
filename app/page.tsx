  ...
  return <main className={`${highContrast ? "high-contrast" : ""} ${largeText ? "large-text" : ""}`}><div className="game-shell">
    {screen === "menu" && <section className="screen menu-screen" aria-label="Menu principal">
      <nav className="menu-tools" aria-label="Opções do jogo">
        <button onClick={() => setModal("avatar")}><img src="/icone-avatar-v1.png" alt="" />Avatar</button><button onClick={() => setModal("achievements")}><img src="/icone-conquistas-v1.png" alt="" />Conquistas</button><button className={!sound ? "sound-muted" : ""} onClick={() => { setSound(!sound); if (!sound) speak("Som ativado"); }} aria-pressed={sound}><img src="/icone-som-v1.png" alt="" />Som</button><button onClick={() => setModal("accessibility")}><img src="/icone-acessibilidade-v1.png" alt="" />Acessibilidade</button><button onClick={() => setModal("help")}><img src="/icone-como-jogar-v1.png" alt="" />Como jogar</button>
      </nav><div className="menu-content"><Macaw message={<><strong>Olá, explorador!</strong><span>Pronto para conhecer o Brasil?</span></>} /><Logo /><button className="start-button" onClick={() => setScreen("journey")}>INICIAR<br />AVENTURA <span aria-hidden="true"></span></button></div><div className="footprints" aria-hidden="true"></div>
    </section>}
