...
function Credits({ onClose }: { onClose: () => void }) {
  ...
  return <><header className="credits-heading"><span aria-hidden="true">★</span><div><h2 id="modal-title">CRÉDITOS</h2><p>Quem criou esta aventura</p></div><span aria-hidden="true">★</span></header><div className="credits-sections"><section className="credits-group credits-management"><h3>GESTÃO E QUALIDADE</h3><div className="credits-grid credits-row">{people.slice(0, 3).map(personCard)}</div></section><section className="credits-group credits-development"><h3>EQUIPE DE DESENVOLVIMENTO</h3><div className="credits-grid credits-row">{people.slice(3, 6).map(personCard)}</div></section><section className="credits-group credits-orientation"><h3>ORIENTAÇÃO</h3><div className="credits-grid credits-row">{people.slice(6).map(personCard)}</div></section></div><aside className="credits-institution" aria-label="Instituição parceira"><img src="/cruzeiro-do-sul-game-v1.png" alt="Símbolo da Cruzeiro do Sul" /><div><small>PROJETO ACADÊMICO</small><strong>CRUZEIRO DO SUL</strong></div></aside><p className="credits-thanks">Obrigado por fazer parte desta aventura pelo Brasil!</p><button className="credits-back" onClick={onClose}>VOLTAR AO MENU</button></>;
}
